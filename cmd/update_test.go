...
func TestRunUpdatePinsSelectedDirectVersionWhenProxyStale(t *testing.T) {
	...
	installFakeGoProxyDirectWithInstall(t, "v0.3.9", "v0.3.10", logPath)
	...
	if !strings.Contains(stdout, "Installing: go install github.com/gurkangul/gg-cli/cmd/gg@v0.3.10") {
		t.Fatalf("stdout did not show pinned install target:\n%s", stdout)
	}
}
...
func installFakeGoProxyDirectWithInstall(t *testing.T, proxyVersion, directVersion, logPath string) {
	t.Helper()
	dir := t.TempDir()
	goPathRoot := filepath.Join(t.TempDir(), "gopath")
	goPath := filepath.Join(dir, "go")
