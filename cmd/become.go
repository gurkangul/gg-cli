...
func runBecomeMaster(cmd *cobra.Command, _ []string) error {
	...
	fmt.Fprintln(out, "────────────────────────────────────────────────────────────────")
	fmt.Fprintln(out)
	fmt.Fprintln(out, "You are now the MASTER session for this project.")
	fmt.Fprintln(out, "Read CLAUDE.md (gg:master-role:begin v3) for your responsibilities.")
	fmt.Fprintln(out, "Your job: spawn workers via gg spawn worker, review their commits,")
	fmt.Fprintln(out, "never write production code (≤5 line cosmetic exception only).")
	fmt.Fprintln(out, "Heartbeat starting now.")
	fmt.Fprintln(out)
