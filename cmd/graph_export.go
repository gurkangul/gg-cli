...
func runGraphExport(cmd *cobra.Command, _ []string) error {
	...
	nodes, edges, err := exportGraphData(ctx, gc, graphExportSymbolCap)
	if err != nil {
		return err
	}
	...
	return printJSON(map[string]any{"output": outPath, "format": "html", "nodes": len(payload.Nodes), "edges": len(payload.Edges), "symbol_view": payload.SymbolView}, func() {
