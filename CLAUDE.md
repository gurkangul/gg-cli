...
- **Keep going.** Finishing a sub-step is not a reason to stop and report. While an
  obvious in-scope step remains in the current objective, take it. Stop for a real
  blocker, an irreversible action, a missing credential, or a product decision that
  is genuinely the user's — not to ask permission for work the objective already
  implies. Simple tasks are inspect → implement → verify, with no plan document.
...
- **Context health.** When you notice the signals — re-reading the same files,
  losing track of what you changed, forgetting decisions already made, contradicting
  rules you read earlier — stop taking on new work. Finish the atomic step you are
  inside, leave the repo building, run the relevant checks, then write the handoff
  **into gg, not into chat**: `gg task ready-for-live --plan`, or `gg tell --task`
  mid-task, carrying objective, branch + commits, changed areas, verified check
  results, decisions made, what is unresolved, the exact next task, constraints to
  preserve, and what must not be redone. A handoff that exists only in a chat
  message is the hidden parallel tracker this project forbids.
