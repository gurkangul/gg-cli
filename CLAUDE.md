...
- Keep going. Finishing a sub-step is not a reason to stop and report; while an obvious in-scope step remains in the current objective, take it. Stop for a real blocker, an irreversible or outward-facing action, a missing credential, or a decision that is genuinely the user's — not to ask permission for work the objective already implies. Match planning to size: a simple task is inspect → implement → verify, with no plan document.
- When your own context degrades — re-reading the same files, losing track of what you changed, forgetting decisions already made, contradicting rules you read earlier — stop taking on new work. Finish the atomic step you are inside, leave the repo building, run the relevant checks, then write the handoff **into gg, not into chat**: `gg task ready-for-live --plan` when the work is ready for review, or `gg tell --task` mid-task, carrying the objective, branch and commits, changed areas, verified results, decisions made, what is unresolved, the exact next step, constraints to preserve, and what must not be redone. A handoff that exists only in a chat message is the hidden parallel tracker this contract already forbids — the next session cannot read your chat.
...
- **You own the outcome, not just your edits.** Use subagents when independent work
  ...
  regressions. Never take a subagent's result on trust — re-verify its claims
  yourself before they become evidence.
