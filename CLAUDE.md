...
- Never take a subagent's result on trust. The review above is delegated to a separate context precisely because it does not share your blind spot — but its findings are claims, not evidence, until you have checked them yourself. Re-run the failing case, read what it cited, reproduce the measurement. Accepting a subagent's verdict unread substitutes one unverified opinion for another and leaves nothing in the ledger a future agent can rely on. Checking a finding is not grounds to overrule it: if you still disagree after checking, that goes back to the reviewer or to the owner, never to your own judgement about your own diff.
...
## WORKING MODE (project-local)

Everything above is regenerated by `gg doctor --check-contract --fix` *and* by the
session-start resync, so edits inside those markers are lost without you running
anything. Project-local rules belong here, below the block. The
contract above and `AGENTS.md` win on anything they already cover — this section
only adds what neither states.

- **You own the outcome, not just your edits.** Use subagents when independent work
  genuinely runs faster in parallel, and prefer a few focused ones over many. Never
  fan out across the same files or tightly coupled areas: on 2026-08-07 two sessions
  writing one working tree broke the repro suite twice and produced two false
  regressions.
- **Debug before delegating.** When a bug arrives, do not ask the user to reproduce,
  bisect, or gather logs. Work the repo, logs, stack traces, failing tests and
  `git log`/`git blame` first; ask only once that evidence is exhausted.
- **Own your breakage.** If your change turns the build, a gate, or CI red, diagnose
  and fix it yourself. Read your own final diff before any handoff — scope creep,
  debug leftovers, dead code, hardcoded values, unintended behavior.
- **Resuming.** A fresh session reads `AGENTS.md`, runs `gg onboard` and
  `gg context --for-task TASK-NNN`, then reconciles it against `git status`, branch and
  recent commits. Do not re-analyse the whole project, and do not redo work the
  ledger already records as verified.
