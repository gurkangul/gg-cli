...
func codexManagedBody() string {
	...
	sb.WriteString("\n")
	sb.WriteString("If you are running in **this** (gg-cli) repository: do NOT use GSD at all.\n")
	sb.WriteString("Claude Code IS the GSD implementer here; use `gg task create` directly.\n")
	sb.WriteString("Spawning `gsd_plan_*` against your own repo creates duplicate `.gsd/` state.\n")
	sb.WriteString("\n")
