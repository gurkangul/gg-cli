...
func codexManagedBody() string {
	...
	sb.WriteString("only see what's in gg. Treat GSD as a local scratchpad or manual helper, not\n")
	sb.WriteString("as the team tracker.\n")
	sb.WriteString("\n")
	sb.WriteString("GSD itself is **not banned**. What is removed is gg-owned orchestration of GSD\n")
	sb.WriteString("tabs, panes, queues, nudges, heartbeats, and worker routing. If you run GSD,\n")
	sb.WriteString("run it manually and copy durable outcomes into gg.\n")
	sb.WriteString("\n")
	sb.WriteString("Rules when GSD is in use:\n")
	sb.WriteString("\n")
	sb.WriteString("- Use `gg task create` for work items that matter to the project.\n")
	sb.WriteString("- Use `gg record` for decisions and `gg tell` for cross-agent messages.\n")
	sb.WriteString("- Summarize useful GSD output back into gg; do not rely on `.gsd/gsd.db` for\n")
