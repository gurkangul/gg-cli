...
func gsdBridgeBlock() string {
	...
	sb.WriteString("- Set `GG_AGENT` to the runtime actually executing gg commands. If this\n")
	sb.WriteString("  terminal is GSD, use a unique `gsd-*` id such as `gsd-myproject-1`;\n")
	sb.WriteString("  if a host agent mirrors GSD output, keep the host runtime id.\n")
