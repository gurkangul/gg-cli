...
// advertise exactly the eight gg_* read tools and zero write tools.
func TestReadOnlyToolSurface(t *testing.T) {
	tools := NewHost().ListTools()
	want := map[string]bool{
		"gg_search": false, "gg_context": false, "gg_impact": false,
		"gg_def": false, "gg_uses": false, "gg_canon": false, "gg_task_get": false, "gg_bug_get": false,
	}
	...
	if len(tools) != 8 {
		t.Fatalf("expected exactly 8 read tools, got %d", len(tools))
	}
}
