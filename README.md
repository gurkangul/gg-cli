...
- An embedding provider: native Ollama (`brew install ollama` + `ollama serve` +
  `ollama pull qwen3-embedding:0.6b`, the default; any Ollama embedding model works —
...
- embeddings via native Ollama (`qwen3-embedding:0.6b` by default; override
  per-shell with `GG_EMBED_MODEL`); if Ollama is unreachable,
