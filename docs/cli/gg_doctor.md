...
```
  ...
  -h, --help                     help for doctor
      ...
      --install-index-hooks      install opt-in git hooks (pre-push/post-merge/post-commit) that run gg index --changed to keep the local CodeGraph fresh; detached + non-blocking (background, logs to .gg/index-hook.log), not a daemon
